...
- **Search grounding is follow-up only.** `build_search_tool()` is passed when `landmark_context` is set, and omitted for identification — recognition is a vision task the search tool cannot help with, and grounding is billed per request.
